...
\note{
  ...
  \begin{itemize}
    \item Without the univalence axiom, it is consistent to assume
    \emph{uniqueness of identity proofs}, i.\,e.\@
    \[ \prod_{X:\U} \prod_{x,y:X} \prod_{p,q:(x=y)} (p=q), \]
    thus collapsing the homotopical universe.
    \item Most results require the univalence axiom.
  \end{itemize}

  \begin{itemize}
    \item \emph{No computational interpretation of the univalence axiom is
...
\note{
  \begin{itemize}
    ...
    to the univalence axiom, there is no hope of mitigating this.
  \end{itemize}
}
...
\note{
  \begin{itemize}
    ...
    \item built-in ``functoriality'', invariance under equivalence
  \end{itemize}
}
