...
\frame[t]{\frametitle{References}
  ...
  \floatbox{-260}{60}{\includegraphics[scale=0.5]{hott-book}}
}
...
Better explain next time or in the slides:

* How to use HoTT for spaces which are externally defined, i.e. not given
  as an inductive type, but given as an ordinary manifold (say).

* How to use HoTT for subjects which have not yet received the HoTT treatment.

* Explain the status of non-continuous functions.
  Explain that the "good old sets" are still there, if you need them.
  But also explain that higher groupoid structures are there, if you need those.

* Explain that by "=", we really mean equality. Explain that homotopic paths
  really are *the same* in HoTT.

* Give examples outside of homotopy theory. (!)

* Concisely explain that there are three ways to read a type:
  as a type, in a logical way, in a geometric/continuous/homotopic way.

* Explain how to define connectedness.

* Maybe: Explain how to prove (x = y) --> (f(x) = f(y)).

* Explain in more detail how identity witnesses can be composed, inverted, and
  so on.

* Maybe: Think about how type families really are fibrations
  (path-lifting property with respect to *any* other type, not just 1).

* Examples for dependent sums: algebraic structures (monoids, ...)

* Everday example for dependent product: sections of vector bundles.

* Briefly show table of known homotopy groups of the spheres (for giving
  context to non-geometers).

* Emphasize that higher groupoidal structure is there when you need it.
  See https://golem.ph.utexas.edu/category/2012/05/what_is_homotopy_type_theory_g.html.

* Emphasize use of HoTT as an internal language.
