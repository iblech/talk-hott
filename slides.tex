...
\note{
  \begin{itemize}
    \item\justifying In the special case that~$B(x) \defeqv B$ does not depend on $x$, the
    \emph{dependent sum} and the \emph{dependent product} simplify to
    \emph{cartesian product} and \emph{function type}, respectively:
    \[ \sum_{x:A} B \equiv A \times B \quad\text{and}\quad
      \prod_{x:A} B \equiv (A \to B). \]

    \item Dependent sums and products are very common in mathematics, even if
    they are not explicitly referred to by name. For instance, a tangential
    vector field on a manifold~$M$ is a value of the dependent
    product~$\prod_{x:M} T_xM$, and the total space of the tangent bundle
    is~$\sum_{x:M} T_xM$. Dependent sums also occur in probability theory, when
    constructing sample spaces for multi-stage experiments.
  \end{itemize}
}

\note{
  \justifying
  The rules governing product types are the following. See the HoTT book for
  explanation.
  \medskip

  \hspace*{-0.8cm}%
  \includegraphics[scale=0.85]{product-types.pdf}
}
...
\note{
  \begin{center}
    \scalebox{0.6}{\input{paths.pspdftex}}
  \end{center}
  \vspace{-1.3em}
...
\note{
  \begin{itemize}
    \item The only interesting feature about a mere proposition is whether it
    is inhabited or not.
    \item The type $\IsMereProp(X)$ is equivalent to $(X \to \IsContr(X))$.
  \end{itemize}
}
...
\frame[t]{\frametitle{How are types like spaces?}
  \begin{itemize}
    ...
    \hil{fibrations}, in that fibers over connected points are equivalent:
...
\note{
  \begin{itemize}
    \item\justifying $\U$ is a \emph{universe}. Its values are, to a first
    approximation, all types.
    \item For types $A$ and $B$, $(A \simeq B)$ is the type of
    \emph{equivalences} between~$A$ and~$B$. See below.
    \item The functions $(x = y) \longrightarrow (f(x) = f(y))$ (for~$x,y:X$) are compatible
    with composition of identity witnesses.
  \end{itemize}
}
...
\frame[t]{\frametitle{How are constructions encoded?}
  \begin{itemize}
    ...
    \[ \fst : \sum_{y:X} (x=y) \to X. \]
  \end{itemize}
  \floatbox{-250}{-45}{\only<2>{\includegraphics[scale=0.5]{look-ma}}}
}

\note{
  Look ma! For doing homotopy theory in HoTT, the following are \emph{not} needed:
  \begin{itemize}
    \item open sets
...
\note{
  The \emph{infinite-dimensional sphere} $S^\infty$ is generated by
  \begin{itemize}
    \item a point $\N : S^\infty$ and
    \item a point $\S : S^\infty$ and
    \item for each $x : S^\infty$, a path $\merid(x) : (N = S)$.
  \end{itemize}
  So~$S^\infty \simeq \Sigma S^\infty$.

  Of course, we can show $S^\infty \simeq \boldsymbol{1}$.
}

\note{
  If~$G$ is a discrete group, the \emph{classifying space}~$BG$ is generated by
  \begin{itemize}
    \item a point $\base : BG$,
    \item a function $\lloop : G \to (\base = \base)$,
    \item a value of type $\lloop(e) = \id$,
    \item a value of type $\prod_{x,y:G} \lloop(x \circ y) = \lloop(y) \ct
    \lloop(x)$, and
    \item ``$BG$ is a $1$-type'': for each $x,y : BG$ and $p,q : (x=y)$ and
    $r,s : (p=q)$, a path $r = s$.
  \end{itemize}

  \scriptsize
  See \url{http://dlicata.web.wesleyan.edu/pubs/lf14em/lf14em.pdf}.
}
...
\note{
  ...
  {\scriptsize The drawing was lifted from the HoTT book.}
}


\section{Further topics}

\subsection{What is path induction?}

\frame[t]{\frametitle{What is path induction?}
  \hil{(Based) path induction} states: Given
  \begin{itemize}
    \item a value~$a$ of a type $A$,
    \item a type family~$C : \prod_{x:A} ((a = x) \to \U)$, and
    \item a value~$c : C(a,\refl_a)$,
  \end{itemize}
  there is a function
  \[ f : \prod_{x:A} \prod_{p:(a=x)} C(x,p) \]
  such that~$f(a,\refl_a) \equiv c$.
}
...
\note{
  \begin{itemize}
    ...
    path starting at~$x$) is connected to the trivial path at~$x$. But this
...
\note{
  \begin{itemize}
    \item\justifying A type~$X$ is \emph{connected} if and only if
    \[ \sum_{x:X} \prod_{y:X} \|x = y\|_{-1}. \]
    \item In logical terms, this is read as follows: A type~$X$ is connected if
    and only if there is some value~$x:X$ such that for any value~$y:X$, it is
    \emph{merely true} that~$x = y$.
    \item In geometric terms, it reads: A space~$X$ is connected if and only if
    there is some point~$x:X$ such that for any point~$y:Y$, there is a path
    connecting~$x$ to~$y$, and this path does not need to depend continuously
    on~$y$.
  \end{itemize}
}
...
\note{
  \begin{itemize}
    \item\justifying Out of a value of~$\|X\|_{-1}$, we cannot extract a
    witness of~$X$, i.\,e.\@ there is no function of type
    \[ \prod_{X:\U} (\|X\|_{-1} \longrightarrow X). \]
    \item Intuitively, if~$\|X\|_{-1}$ is inhabited, then we know that there
    must be some witnesses of~$X$, but generally, we cannot choose a specific
    such witness: By our general principles, this witness would depend in a
    continuous way on~$X$, but this is too much to ask for.
    \item However, by the universal property of~$(-1)$-truncation, to prove an
    implication of the form $\|X\|_{-1} \longrightarrow Y$ where~$Y$ is itself
    a mere proposition, it suffices to prove the a priori weaker statement $X
    \longrightarrow Y$. Intuitively, in this case we \emph{can} make the
    choice, because the produced value of~$Y$ does not depend on it.
  \end{itemize}
}

\note{
  \begin{itemize}
    \item\justifying By circle induction, an equivalent definition of the
...
%   \item This is one of many places where we can clearly see that homotopy
%   type theory provides a synthetic treatment of \emph{homotopy types}
...
\note{
  ...
  \definecolor{xA}{rgb}{1., 0.85, 0.85}
  \definecolor{xB}{rgb}{1., 0.988462, 0.85}
  \definecolor{xC}{rgb}{0.873077, 1., 0.85}
  \definecolor{xD}{rgb}{0.85, 1., 0.965385}
  \definecolor{xE}{rgb}{0.85, 0.896154, 1.}
  \definecolor{xF}{rgb}{0.942308, 0.85, 1.}
  \definecolor{xG}{rgb}{1., 0.85, 0.919231}
  \definecolor{xH}{rgb}{1., 0.919231, 0.85}
  \definecolor{xI}{rgb}{0.942308, 1., 0.85}
  \definecolor{xJ}{rgb}{0.85, 1., 0.896154}
  \definecolor{xK}{rgb}{0.85, 0.965385, 1.}
  \definecolor{xL}{rgb}{0.873077, 0.85, 1.}
  \definecolor{xM}{rgb}{1., 0.85, 0.988462}

  \newcommand{\cA}{\colorbox{xG}{\hbox to 20pt {\hfil$0$\hfil}}}
  \newcommand{\cB}{\colorbox{xH}{\hbox to 20pt {\hfil$\ZZ$\hfil}}}
  \newcommand{\cC}{\colorbox{xI}{\hbox to 20pt {\hfil$\ZZ_{2}$\hfil}}}
  \newcommand{\cD}{\colorbox{xJ}{\hbox to 20pt {\hfil$\ZZ_{2}$\hfil}}}
  \newcommand{\cE}{\colorbox{xK}{\hbox to 20pt {\hfil$\ZZ_{24}$\hfil}}}
  ...
  \newcommand{\OPTspherescolwidth}{25pt}
  ...
  $\pi_{10}$ & $0$     & $\ZZ_{15}$ & $\ZZ_{15}$ & {$\ZZ_{24} {\times} \ZZ_{3}$} & $\ZZ_{2}$ & \cF & \cE & \cD \\ \addlinespace[3pt]
  $\pi_{11}$ & $0$     & $\ZZ_{2}$  & $\ZZ_{2}$  & $\ZZ_{15}$ & $\ZZ_{2}$ & $\ZZ$ & \cF & \cE \\ \addlinespace[3pt]
  $\pi_{12}$ & $0$     & $\ZZ_{2}^{2}$ & $\ZZ_{2}^{2}$ & $\ZZ_{2}$ & $\ZZ_{30}$ & $\ZZ_{2}$ & \cG & \cF \\ \addlinespace[3pt]
  $\pi_{13}$ & $0$     & {$\ZZ_{12} {\times} \ZZ_{2}$} & {$\ZZ_{12} {\times} \ZZ_{2}$} & $\ZZ_{2}^{3}$ & $\ZZ_{2}$ & $\ZZ_{60}$ & $\ZZ_{2}$ & \cG \\ \addlinespace[3pt]
  \bottomrule
  \end{tabular}
...
\subsection{What are applications?}

\frame[t]{\frametitle{What are applications?}
  Homotopy type theory \hil{subsumes all of classical mathematics}, by
  using the fragment of discrete types.

  The following subjects have received special treatment:

  \begin{itemize}
  \item Homotopy theory (duh)
  \item Category theory
  \item Data type theory
  \item Your subject here!
  \end{itemize}

  Also, HoTT can be used as the \hil{internal language} of
  $(\infty,1)$-toposes.
}

\note{
  \justifying
  There is also lots of activity related to homotopy type theory in type
  theory, $(\infty,1)$-category theory, the theory of simplicial and cubical
  sets, computer-aided reasoning, \ldots
}

\note{
  \begin{itemize}
    \item\justifying In homotopy type theory, isomorphic objects of categories \emph{are
    equal}.
    \item Like in classical category theory, any fully faithful and essentially
    surjective functor is an equivalence of categories. Unlike in the classical
    approach, this is proven by a simple appeal to the axiom (really theorem)
    of \emph{unique choice}.
    \item Equivalent categories are equal.
  \end{itemize}
}

\note{
  \begin{itemize}
    \item\justifying ``Recall'' that data types can be modeled as initial algebras for
    \emph{polynomial functors}.
    \item For instance, the type~$\List(X)$ of lists of type of~$X$ is the
    initial algebra for the functor
    \[ {?} \longmapsto 1 + X \times {?}. \]
    \item Formally differentiating the fixed-point equation $\List(X) = 1 + X
    \times \List(X)$ gives
    \[ \partial \List(X) = \List(X) + X \times \partial\List(X). \]
    Formally solving this equation gives
    \[ \partial \List(X) = \List(X) \times \List(X). \]
    \item This data type is the \emph{zipper for lists}, i.\,e.\@ the type of
    \emph{one-hole contexts} of lists.
  \end{itemize}
}

\note{
  \begin{itemize}
    \item\justifying $\partial X^n = n X^{n-1}$: This makes sense.
    \item What's the antiderivative of~$X^n$? What should $\frac{1}{n+1}
    X^{n+1}$ be?
    \item The type of \emph{cycles} of length $n+1$!
    \item Unfortunately, this type can not be modeled in the standard approach.
    \item It can in homotopy type theory, by simply relaxing a part of the
    definition of polynomial functors (not shown here) to no longer restrict to
    \emph{sets}, but to allow arbitrary \emph{types}.
  \end{itemize}

  {\scriptsize See \url{http://www.cs.nott.ac.uk/~txa/talks/lyon14.pdf}.\par}
}


\backupstart

\againframe<1-2>{awesome}
...
\frame[t]{\frametitle{References}
  \begin{itemize}
    ...
    \url{https://www.andrew.cmu.edu/user/awodey/hott/CMUslides.pdf}
    \url{http://home.sandiego.edu/~shulman/papers/hott-grothendieck.pdf}\par}

    \item hott-amateurs mailing list

    {\tiny\url{https://groups.google.com/d/forum/hott-amateurs}\par}
  \end{itemize}

  \floatbox{-253}{65}{\includegraphics[scale=0.5]{hott-book}}
}

\note{
  \justifying
  An application in string theory: \url{
  https://golem.ph.utexas.edu/category/2012/05/what_is_homotopy_type_theory_g.html}
  \bigskip

  There is a vibrant community of \emph{HoTT amateurs} -- people without a
  formal training in mathematics who nevertheless are interested in homotopy
  type theory. Most of these people have a background in programming,
  particularly with fancy languages like Haskell.
  \par
}
...
\frame[t]{\frametitle{How can we calculate~$\pi_1(S^1)$?}
  By circle induction, we define a type family~$\code$ which will turn out to
  be the universal covering space (think~$\RR \to S^1$):
  \[ \code : S^1 \to \U,\ \base \mapsto \ZZ,\ \lloop \mapsto \ua(\succ). \]
  Here,~$\ua(\succ) : (\ZZ = \ZZ)$ is the identity witness corresponding to the
  equivalence~$\succ : \ZZ \to \ZZ$.
}

\note{
  \justifying
  Thought of a fibration,~$\code$ looks like this: Its fiber over~$\base$
  is~$\ZZ$. A number~$v : \code(\base)$ is transported along~$\lloop$ to~$v +
  1$.
}
...
\note{
  ...
  Define:
  \[ \begin{array}{@{}ccrcll@{}}
    f &:& \ZZ &\longrightarrow& (\base = \base), & n \longmapsto \lloop^n \\[0.4em]
    g &:& (\base = \base) &\longrightarrow& \ZZ, & p \longmapsto \code(p)(0)
  \end{array} \]

  Verify~$g \circ f = \id$ by induction. But there is no principle which would
  help in proving that~$f \circ g = \id$. For this, need to \emph{generalize};
  define by circle induction on~$x : S^1$
  \[ \begin{array}{@{}ccrcl@{}}
    f_x &:& \code(x) &\longrightarrow& (\base = x) \\[0.4em]
    g_x &:& (\base = x) &\longrightarrow& \code(x)
  \end{array} \]
  such that~$f_\base \equiv f$ and~$g_\base \equiv g$.

  Then~$\prod_{x:S^1} (f_x \circ g_x = \id)$ can be proven by path induction.

  In particular, we obtain~$(\base = \base) \simeq \ZZ$. Taking
  the~$0$-trun\-ca\-tion, we obtain
  \[ \pi_1(S^1,\base) \defeqv \|(\base = \base)\|_0 \simeq
    \|\ZZ\|_0 \simeq \ZZ. \]
}
...
* Recap types and values.
  * Examples for dependent sums: algebraic structures (monoids, ...)
  * Everday example for dependent product: sections of vector bundles.

* Show rules for product types.

* Recall the three perspectives on types:
  * containers of values
  * logical statements about set-like things ("for all", "there exists")
  * statements about spaces ("one can continuously ...")

* Discuss question of "cardinality of types":
  * First note that for arbitrary types, the question does not make sense,
    because it's not clear how to regard nontrivial identity types.
  * But for sets, it does and we have the usual theory of cardinal numbers.

* Explain that by "=", we really mean equality.
  Discuss composition of paths and path induction.
  Give homotopy-theoretic justification for path induction.
  Explain how to prove (x = y) --> (f(x) = f(y)).

* Recap the definition of the circle and other spaces.
  Carefully explain how higher paths which are not specified in an inductive
  definition come into existence.

* Explain how to use inductive definitions: by their induced principle.
  So explain circle induction. Give geometric justification.

* Explain truncation of types. With this, give the definition of the
  fundamental group. Do not yet prove that pi_1(S^1) ~ Z. Briefly show the
  table of known homotopy groups. Discuss how to formalize connectedness.

* Finally, state the univalence axiom. Discuss consequences:
  * Isomorphic groups/vector spaces/... are really equal.
  * Any construction and any statement respects equivalence.
  Also mention that no computational interpretation is known yet.

* If wanted, prove pi_1(S^1) ~ Z.

* Applications:
  * Everything of mathematics. You don't have to use the higher features.
    Just encode everything as 0-types. Explain the status of non-continuous functions.
    Explain that the "good old sets" are still there, if you need them.
  * Homotopy theory. Already discussed that.
    Note that one has to have some internal description of spaces which one
    want to study. Briefly mention models here.
  * String theory: Urs Schreiber, Mike Shulman.
    https://golem.ph.utexas.edu/category/2012/05/what_is_homotopy_type_theory_g.html
  * Category theory: Isomorphic objects are equal, fully faithful and
    essentially surjective functors are equivalences, equivalent categories are
    equal.
  * Data type theory: Have more antiderivatives of data types.
  * HoTT as internal language
  * Your application here!


To do:

* Check Bene's example.
